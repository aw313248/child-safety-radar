    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-warm, #FFF6E6)', fontFamily: 'var(--font-heihei, sans-serif)' }}>
      ...
      <header style={{
        ...
        <Link href="/" style={{
          ...
          color: 'var(--ink-hex)', textDecoration: 'none', letterSpacing: '-0.02em',
        }}>
          ← 回首頁
        </Link>
        <span style={{ fontSize: 13, color: 'var(--ink-hex)', opacity: 0.6 }}>⚑ 我的黑名單</span>
      </header>

      <main style={{ maxWidth: 720, margin: '0 auto', padding: '40px 24px 80px' }}>
        <h1 style={{
          fontSize: 24, fontWeight: 900,
          color: 'var(--ink-hex)', letterSpacing: '-0.04em', marginBottom: 6,
        }}>
          我的黑名單
        </h1>
        <p style={{ fontSize: 13, color: 'var(--ink-hex)', opacity: 0.5, marginBottom: 32 }}>
        ...
        {items.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <p style={{ fontSize: 16, color: 'var(--ink-hex)', fontWeight: 600 }}>
              目前沒有列入黑名單的頻道
            </p>
            <p style={{ fontSize: 14, color: 'var(--ink-hex)', opacity: 0.5, marginTop: 8 }}>
          ...
          <ul style={{
            ...
            {items.map(item => (
              <li key={item.channelId} style={{
                ...
                <div style={{ fontSize: 12, color: 'var(--ink-hex)', opacity: 0.5, marginTop: 3 }}>
                  {item.channelId}
                  {item.blacklistedAt && (
